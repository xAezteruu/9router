...
import { BIN_PATH, isValidBinary, runDownload, getInstalledCloudflaredVersion, resolveCloudflaredBin } from "./cloudflaredUpdateUtil.js";
import { DATA_DIR } from "@/lib/dataDir.js";

// BIN_PATH / validity / download live in cloudflaredUpdateUtil.js (single source
// of truth shared with cloudflaredUpdater.js), re-exported here for existing importers.
export { BIN_PATH, isValidBinary, getInstalledCloudflaredVersion };
...
async function _ensureCloudflared() {
  // Preferred: npm package binary (`cloudflared` dep) — native, versioned with
  // the app, no manual download. Updated via plain `npm update cloudflared`.
  const pkgBin = resolveCloudflaredBin();
  if (pkgBin) return pkgBin;

  // Fallback: DATA_DIR copy (legacy path — previously installed via updater)
