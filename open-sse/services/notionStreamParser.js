 ...
 * Ported from OmniRoute open-sse/services/notionStreamParser.ts.
 *
 * Parses Notion's undocumented streaming response format (legacy rich-text
 * tuples, patch-start / patch ops, and terminal record-map agent-inference
 * steps) into plain text, and detects in-band Notion error objects (often
 * shipped with HTTP 200).
 */

/** Strips lang tags / BOM noise Notion sometimes wraps assistant text in. */
export function sanitizeNotionAssistantText(text) {
  if (!text) return "";
