 ...
 *
 * Notion AI has no public, documented inference API. This executor reverse-
 * engineers the same cookie-authenticated internal endpoint used by open-source
 * bridges (notion2api / Notion2API-go): a `token_v2` session cookie posted to
 ...
 *   - response is NDJSON patch-start / patch / record-map
 *
 ...
 * TLS: requests go through ExtremeRouter's tlsFetch (wreq-js Chrome JA3
 * impersonation, same as claude-web/chatgpt-web) — plain undici fetch is
 * rejected by Notion's edge with in-band `temporarily-unavailable`.
 *
 * Ported from OmniRoute open-sse/executors/notion-web.ts.
 */
...
const NOTION_REQUEST_TIMEOUT_MS = 180_000;

const CFG = PROVIDERS["notion-web"];

// ─── Test hook ──────────────────────────────────────────────────────────────
// OmniRoute ships the same override for its notionTlsClient; without it the
// executor tests would need a live wreq-js session. When set, it replaces
// tlsFetch for runInferenceTranscript only.
let tlsFetchOverrideForTesting = null;

export function __setTlsFetchOverrideForTesting(fn) {
  tlsFetchOverrideForTesting = fn;
}
...
export function extractSpaceIdFromCookie(cookie) {
 ...
 */
function buildNotionInferenceRequestBody({ spaceId, userId, threadId, transcript, createThread, agent }) {
...
function buildNotionExecuteHeaders({ cookie, spaceId, userId, agent }) {
  const isCustom = Boolean(agent?.workflowId);
  // Browser uses /agent/<workflowId without dashes>?wfv=chat for custom agents.
  const agentPathId = (agent?.workflowId || "").replace(/-/g, "");
...
export function normalizeNotionWorkflowId(raw) {
 ...
 */
export function resolveNotionAgentOptions(credentials, cookie) {
 ...
 */
async function sendNotionInferenceRequest({ reqBody, reqHeaders, signal }) {
  ...
  // Chrome-JA3 tls path for runInferenceTranscript (falls back to plain fetch
  // internally when wreq-js is unavailable).
  ...
  let rawText = "";
  try {
    const upstreamFetch = tlsFetchOverrideForTesting ?? tlsFetch;
    const tlsRes = await upstreamFetch(NOTION_URL, {
      method: "POST",
      headers: reqHeaders,
    ...
    // tlsFetch returns a real Response (`.text()` fn); the test override and
    // OmniRoute's tls-client return `{ text: string }`. Support both.
    rawText =
      typeof tlsRes.text === "function"
        ? await tlsRes.text().catch(() => "")
        : String(tlsRes.text ?? "");
  } catch (err) {
    return {
...
export class NotionWebExecutor extends BaseExecutor {
  ...
  async execute(input) {
    ...
    // Optional custom agent (workflowId). Empty → default Notion AI.
    ...
    // Namespace the thread cache PER CALLER (hash of the caller's cookie) AND by
    // custom agent, so (a) two users of the same Notion space never share a cached
    // thread (cross-tenant IDOR, #7900 review) and (b) default AI and agents never
    // share threads.
    ...
    const isFailedAttempt = (attempt) => !attempt.ok;

    const runOnce = async ({ createThread: createThreadArg, threadId: threadIdArg }) => {
      const transcript = buildNotionTranscript(messages, {
        notionModel: notionCodename || undefined,
        spaceId,
        userId: userId || undefined,
        agent,
        isFollowUp: !createThreadArg,
      });
      const reqBody = buildNotionInferenceRequestBody({
        ...
        transcript,
        createThread: createThreadArg,
        agent,
      });

      if (createThreadArg) {
        notionThreadMarkCreateAttempted(rootKey, threadIdArg);
      }
    ...
    if (isFailedAttempt(attempt) && attempt.retryable) {
      const delayMs =
        process.env.NODE_ENV === "test" || process.env.VITEST ? 20 : 700 + Math.floor(Math.random() * 400);
